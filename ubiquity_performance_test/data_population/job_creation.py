...
def create_tsv_jobs(  # noqa: PLR0913
    total_pcards: int,
...
def init_jobs(
    total_users: int, total_users_history: int, total_pcard_history: int, total_mcard_history: int
) -> list[dict]:
    jobs = []
    job_id = 0
    user_index = 1
...
def fix_indexes_for_card_jobs(jobs: list[dict], card_type: str) -> list[dict]:
...
def process_enough_cards_per_service(  # noqa: PLR0913
    jobs: list[dict],
    card_type: str,
    remaining_cards: int,
    total_cards: int,
...
def add_card_info_to_jobs(
    jobs: list[dict], card_type: str, total_cards: int, cards_per_service: int, total_users: int
) -> list[dict]:
