...
from ubiquity_performance_test.data_population.database_tables import HadesTables
from ubiquity_performance_test.data_population.row_generation import create_mcard
from ubiquity_performance_test.data_population.tsv_generation.common import delete_old_tsv_files, write_to_tsv_part

cores = multiprocessing.cpu_count()


def create_transaction_tsv_job(job: dict) -> None:
    job_id = job["job_id"]
    start_pk = job["start"]
    count = job["count"]
...
def create_transaction_tsv_files(total_transactions: int) -> None:
