...
from ubiquity_performance_test.data_population.data_population_config import DataConfig
...
from ubiquity_performance_test.data_population.fixtures.payment_scheme import ALL_PAYMENT_PROVIDER_STATUS_MAPPINGS
from ubiquity_performance_test.data_population.job_creation import (
    MCARDS_PER_SERVICE,
    PCARDS_PER_SERVICE,
    CardTypes,
    cores,
...
from ubiquity_performance_test.data_population.row_generation import (
    create_association,
    create_channel,
    create_mcard,
    create_pcard,
...
from ubiquity_performance_test.data_population.tsv_generation.common import delete_old_tsv_files, write_to_tsv_part
...
class Counters:
    def __init__(self, job: dict) -> None:
        ...
        self.membership_card_associations: list = []
        self.membership_card_answers: list = []
        self.payment_cards: list = []
        self.payment_card_associations: list = []
        self.pll_links: list = []
        self.pll_user_associations: list = []
        self.vop_activation_dict: dict = {}
    ...
    def write_part_to_csv(self) -> None:
...
def create_channel_tsv_files() -> None:
...
def create_payment_scheme_tsv_files() -> None:
    issuer_names = ["Barclays", "Performance"]
    # note: list(starmap(...)) is also faster than using a list comprehension
    issuers = list(starmap(create_pcard.issuer, enumerate(issuer_names, 1)))
...
def create_membership_plan_tsv_files(total_plans: int, real_plans: bool = False) -> None:  # noqa: PLR0915
    ...
    for count in mcard_range:
        ...
        elif count % 2 == 0:  # Even-numbered plans are non-voucher
...
def create_service_mcard_and_pcard_tsv_files(data_config: DataConfig) -> None:
...
def create_service_mcard_and_pcard_job(  # noqa: PLR0915
    total_plans: int, total_mcards: int, transactions_per_mcard: int, job: dict
) -> None:
    ...
    for service_count in range(0, job["count"]):
        ...
        for _mcard_count in range(0, MCARDS_PER_SERVICE):
        ...
        for _pcard_count in range(0, PCARDS_PER_SERVICE):
...
def create_remaining_mcards_and_pcards(  # noqa: PLR0913
    mcard_start: int,
    pcard_start: int,
    mcard_count: int,
    pcard_count: int,
...
def create_tsv_files(data_config: DataConfig) -> None:
