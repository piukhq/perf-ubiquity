...
from ubiquity_performance_test.angelia_token_generation import set_channels, tokens
from ubiquity_performance_test.locust_angelia.database.session import db_session
from ubiquity_performance_test.locust_angelia.user_behaviour import UserBehavior, set_retry_and_timeout
from ubiquity_performance_test.locust_config import set_task_repeats
from ubiquity_performance_test.vault import vault_secrets


class WebsiteUser(HttpUser):
    vault_secrets.trigger_loading(channel_info=True, aes_key=True)
...
def on_test_start(environment: Environment, **kwargs: Any) -> None:  # noqa: ANN401, ARG001
    logger.info("Test started")
    tokens.generate_tokens(environment)


@events.test_stop.add_listener
def close_db_session(environment: Environment, **kwargs: Any) -> None:  # noqa: ANN401, ARG001
    db_session.close()
